<template>
  <div class="photographer-review-container">
    <h2>摄影师入驻审核</h2>
    ...
    <el-card class="table-card">
      <el-table :data="applicationList" style="width: 100%" v-loading="loading">
        ...
        <el-table-column prop="portfolio" label="作品集链接" min-width="150">
          <template #default="scope">
            <a v-if="scope.row.portfolio" :href="scope.row.portfolio" target="_blank" class="portfolio-link">
              查看作品
            </a>
            <span v-else>-</span>
          </template>
    ...
    <el-dialog v-model="viewDialogVisible" title="申请详情" width="600px">
      <div class="detail-content" v-if="currentApplication">
        <el-descriptions :column="2" border>
          ...
          <el-descriptions-item label="身份证号">{{ currentApplication.idNumber }}</el-descriptions-item>
          <el-descriptions-item label="手机号">{{ currentApplication.phone }}</el-descriptions-item>
          <el-descriptions-item label="擅长风格">{{ currentApplication.styles }}</el-descriptions-item>
          ...
          <el-descriptions-item label="作品集链接" :span="2">
            <a v-if="currentApplication.portfolio" :href="currentApplication.portfolio" target="_blank">
  ...
  setup() {
    ...
    const fetchApplications = async () => {
      ...
      try {
        ...
        const response = await axios.get('http://localhost:8086/admin/api/photographer/application/pending', {
          params
        })
    ...
    const handleApprove = async (row) => {
      try {
        await ElMessageBox.confirm('确认通过该摄影师入驻申请?', '审核确认', {
          confirmButtonText: '确认',
          cancelButtonText: '取消',
          type: 'warning'
        })

        const response = await axios.post('http://localhost:8086/admin/api/photographer/application/audit', {
          id: row.id,
          status: 1,
...
<style scoped>
.photographer-review-container {
  padding: 20px;
}

.photographer-review-container h2 {
  margin-bottom: 20px;
  color: #303133;
}
